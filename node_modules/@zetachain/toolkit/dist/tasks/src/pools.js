...
const main = async (args, hre) => {
    ...
    const wzeta = (0, protocol_contracts_1.getAddress)("zetaToken", `zeta_${client.network}`);
    if (!wzeta) {
    ...
    const poolsWithSymbolsAndDecimals = pools.map((pool) => {
        ...
        pool.t0.reserve = (0, utils_1.formatUnits)(pool.t0.reserve, t0Info.decimals);
        pool.t1.reserve = (0, utils_1.formatUnits)(pool.t1.reserve, t1Info.decimals);
        return {
            ...pool,
            t0: { ...pool.t0, ...t0Info },
            t1: { ...pool.t1, ...t1Info },
